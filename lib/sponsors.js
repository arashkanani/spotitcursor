...
const SEED_ROOT = path.join(__dirname, "..", "seed");
const SEED_SPONSORS_DIR = path.join(SEED_ROOT, "sponsors");
const SEED_INDEX_PATH = path.join(SEED_ROOT, "sponsors-index.json");
...
function readIndex() {
  ...
  if (!fs.existsSync(INDEX_PATH)) {
    fs.writeFileSync(INDEX_PATH, JSON.stringify(defaultIndex(), null, 2), "utf8");
  }
  try {
...
function importSeedSponsors() {
  if (!fs.existsSync(SEED_INDEX_PATH) || !fs.existsSync(SEED_SPONSORS_DIR)) {
    return { imported: 0, sponsors: [] };
  }

  let seedIndex;
  try {
    seedIndex = JSON.parse(fs.readFileSync(SEED_INDEX_PATH, "utf8"));
  } catch (_error) {
    return { imported: 0, sponsors: [] };
  }
  ...
  for (const seedSponsor of seedIndex.sponsors || []) {
    const seedShapesDir = path.join(SEED_SPONSORS_DIR, seedSponsor.id, "shapes");
    if (!fs.existsSync(seedShapesDir)) continue;

    const destShapesDir = getSponsorShapesDir(seedSponsor.id);
    fs.mkdirSync(destShapesDir, { recursive: true });

    const seedFiles = fs
      .readdirSync(seedShapesDir)
      .filter((f) => f.toLowerCase().endsWith(".png"));

    for (const file of seedFiles) {
      const destFile = path.join(destShapesDir, file);
      if (!fs.existsSync(destFile)) {
        fs.copyFileSync(path.join(seedShapesDir, file), destFile);
        importedFiles += 1;
      }
    }

    if (!index.sponsors.some((s) => s.id === seedSponsor.id)) {
      index.sponsors.push({
        id: seedSponsor.id,
        name: seedSponsor.name,
        createdAt: seedSponsor.createdAt || new Date().toISOString(),
        updatedAt: seedSponsor.updatedAt || new Date().toISOString()
      });
      touched.push(seedSponsor.id);
    }
  }

  const hadCustomSponsors = index.sponsors.some(
    (s) => s.id !== "ikea" && s.id !== "oordoo"
  );
  if (!index.activeSponsorId && !hadCustomSponsors) {
    const preferred = seedIndex.activeSponsorId;
    if (preferred && index.sponsors.some((s) => s.id === preferred)) {
      index.activeSponsorId = preferred;
    } else {
      const ready = index.sponsors.find(
        (s) => countShapes(s.id) >= MIN_SHAPES_PER_SPONSOR
      );
      if (ready) index.activeSponsorId = ready.id;
    }
  }

  if (importedFiles > 0 || touched.length > 0) {
    writeIndex(index);
  }

  return { imported: importedFiles, sponsors: touched };
}
...
function initializeSponsors() {
  ensureDataDirs();
  const seedResult = importSeedSponsors();
  if (seedResult.imported > 0) {
    console.log(
      `Seeded ${seedResult.imported} sponsor shape file(s) from bundled seed pack.`
    );
  }
