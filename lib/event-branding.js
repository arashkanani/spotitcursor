...
function readEventConfig() {
  try {
    ...
    const theme = themes.normalizeTheme({
      ...
      themeBackground: raw.themeBackground === "custom" || backgrounds.length
        ? "custom"
        : raw.themeBackground
    });
...
function writeEventConfig(config) {
  ...
  const theme = themes.normalizeTheme({
    ...config,
    themeBackground: config.themeBackground === "custom" || backgrounds.length
      ? "custom"
  ...
  const savedBackgrounds = theme.themeBackground === "custom" ? backgrounds : [];
  const primaryTransform = primaryBackgroundTransform(backgrounds);
  const payload = {
