...
function readEventConfig() {
  try {
    ...
    return {
      ...
      customBackgroundUrls: backgrounds.map((entry) => entry.url),
      customBackgrounds: backgrounds,
      selectedBackgroundUrl: stripBackgroundUrlQuery(
        raw.selectedBackgroundUrl || primaryBackgroundUrl(backgrounds)
      ),
...
function writeEventConfig(config) {
  ...
  const payload = {
    ...
    selectedBackgroundUrl: stripBackgroundUrlQuery(
      config.selectedBackgroundUrl || primaryBackgroundUrl(savedBackgrounds)
    ),
...
function isAllowedBackgroundFilename(name) {
  return /^bg-[\w-]+\.(png|jpe?g|webp)$/i.test(name)
...
function backgroundFilenameFromUrl(url) {
  ...
  if (!isAllowedBackgroundFilename(name)) {
    return null;
  }
  return name;
}

/** Resolve stored file path for a background URL (global or per-user). */
function resolveBackgroundFilePath(url, backgroundsRoot) {
  const base = stripBackgroundUrlQuery(url);
  if (!base || !backgroundsRoot) return null;
  const userMatch = base.match(/^\/event-backgrounds\/users\/([a-f0-9-]+)\/([^/]+)$/i);
  if (userMatch) {
    const [, userId, filename] = userMatch;
    if (!isAllowedBackgroundFilename(filename)) return null;
    return path.join(backgroundsRoot, "users", userId, filename);
  }
  const filename = backgroundFilenameFromUrl(url);
  if (!filename) return null;
  return path.join(backgroundsRoot, filename);
}
