...
const DEFAULT_CIRCLE_SIZE = "standard";
const CIRCLE_SIZES = new Set(["standard", "large", "xlarge", "venue"]);

function normalizeCircleSize(value) {
  const size = String(value || "").trim().toLowerCase();
  return CIRCLE_SIZES.has(size) ? size : DEFAULT_CIRCLE_SIZE;
}
...
function readEventConfig() {
  try {
    ...
    return {
      ...
      circleSize: normalizeCircleSize(raw.circleSize)
    };
  } catch (_error) {
...
function writeEventConfig(config) {
  ...
  const payload = {
    ...
    circleSize: normalizeCircleSize(config.circleSize),
